...
export async function create({
  id, tokenizerId, batchIdSize, batchIndexSize, batchTokenCount
}) {
  if(!id) {
    // get next version ID
...
export async function upsertVersionForTokenizer({tokenizerId}) {
  ...
  while(!tokenVersion) {
    ...
    const {batchIdSize, batchIndexSize, batchTokenCount} =
      record.tokenVersionOptions.options;
    try {
      tokenVersion = await create(
        {tokenizerId, batchIdSize, batchIndexSize, batchTokenCount});
    } catch(e) {
      if(e.name !== 'DuplicateError') {
        throw e;
      }

  ...
  while(true) {
    ...
    if(levelOfAssurance < tokenBatch.minAssuranceForResolution) {
      throw new BedrockError(
        'Minimum level of assurance not met.',
        'NotAllowedError', {
          levelOfAssurance,
...
/**
 * Updates a token batch based off of `internalId`.
 *
 * @param {object} options - Options to use.
 ...
 */
export async function updateBatches({internalId, minAssuranceForResolution}) {
  const collection = database.collections['tokenization-tokenBatch'];
  const query = {
  ...
  for(const record of records) {
    const updateQuery = {
      'tokenBatch.id': record.tokenBatch.id
    };
    const $set = {
      'meta.updated': Date.now(),
    ...
    result = await collection.updateOne(
      updateQuery, {$set}, database.writeOptions);
  }

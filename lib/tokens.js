...
import {constants, appContextMap, documentLoader} from 'cit-context';

const {CIT_CONTEXT_URL} = constants;
...
export async function registerDocumentAndCreate(
  ...
  while(true) {
    ...
    if(registrationResult.registration.internalId !== internalId) {
      continue;
    }
...
async function _createToken({
  ...
  const token = await cborldEncode({
    appContextMap,
    jsonldDocument,
    // FIXME: currently wrapping correct documentLoader function with this,
    //   just pass in documentLoader when cborld module is fixed
    documentLoader: async url => {
      const {document: CIT_CONTEXT} = await documentLoader(url);
...
async function _createBatch({internalId, batchVersion, tokenCount = 0}) {
  ...
  if(remainingTokenCount === 0) {
    return _insertBatch({id, internalId, batchVersion, tokenCount});
  }
  ...
  const [record] = await Promise.all([
    _insertBatch({id, internalId, batchVersion, tokenCount}),
...
async function _insertBatch({id, internalId, batchVersion, tokenCount}) {

...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  await database.createIndexes([{
    ...
    // this does not include the shard key; it is not expected to be used in
    // the hottest code paths, so a "scatter-gather" approach is acceptable;
    // used to find token batches that must be invalidated when the minimum
    // assurance for resolution for an entity increases
    collection: 'tokenization-tokenBatch',
...
export async function registerDocumentAndCreate(
  ...
  while(true) {
    ...
    const [registrationResult, createResult] = await Promise.all([
      registerPromise,
      create(
        {internalId, attributes, tokenCount, registerPromise, newRegistration})
    ]);
...
export async function create({
  ...
  minAssuranceForResolution, registerPromise, newRegistration
} = {}) {
...
export async function resolve({requester, token, levelOfAssurance}) {
  ...
  while(true) {
    ...
    // FIXME: fetch `minAssuranceForResolution` via `internalId` from
    // openTokenBatch collection instead ... while the other code here
    // is executing ... including the other database calls to get a pairwise
    // token/create one ... so that there is minimal latency via parallelism
    // ... THEN, before returning the pairwise token, check that the level
    // of assurance is acceptable, this will also, in effect, use up that
    // token for the party that tried to resolve it, even if it, in the end,
    // cannot be resolved; that way it cannot be resolved by another party
    // (which improves privacy) *and* is more performant since we can avoid
    // state partitioning and scatter-gather queries
...
/**
 * Private function that directly sets the `minAssuranceForResolution` on the
 * token batches associated `internalId`. Exported for internal use and testing
 * purposes only.
 *
 * @param {object} options - Options to use.
 ...
 * @returns {Promise<boolean>} Returns true if update occurred.
 */
export async function _setMinAssuranceForResolution({
...
async function _createBatch({
  ...
  const [record] = await Promise.all([
    ...
    entities.setOpenTokenBatchId({internalId, batchId: id})
  ]);
  return record;
}
...
async function _getOpenBatch({
  ...
  // FIXME: we probably need to return the entity record from this function
  // ... one potential problem is that we may be concurrently upserting the
  // entity ... we would want to have a wait to await that here
  while(true) {
    const {unfilledRecord, entityRecord} = await _getUnfilledBatch(
      {internalId, batchVersion});
    ...
    if(!batchRecord) {
      ...
      /* Note: Since we just created a new batch record, we must account for
      the case where we created one while another process was attempting to
      invalidate all existing batches. That other process could fail to see
      and invalidate the one we just created, so we must ensure that we have
      not hit that case.
      ...
      invalidated token batches while we created our batch. We must not
      issue tokens from our batch when this occurs.
      ...
      `entityRecord` to be `null`. If both of these things are true, it means
      one of two things:

      1. The entity is new and we have raced ahead in creating its first
      token batch before it was registered (which is happening concurrently);
      we will know we are in this case when `newRegistration` is `true`.

      2. The entity has expired and will be assigned a new `internalId`.

      In case 1, we can safely assume that `batchInvalidationCount` would have
      been `0` to start, so we will await `registerPromise` and then fetch the
      entity record to ensure its still `0`.

      In case 2, we simply return because it is safe to assume that the caller
      will need to call `create` again with a new `internalId`. The batch
      we created will eventually be garbage collected.

      Now, if `entityRecord` is `null` and `registerPromise` is not set, then
      ...
      if(entityRecord) {
        ...
        // misuse of the API
        throw new Error(
          'Entities must be registered prior or concurrently with token' +
          'creation.');
      ...
      // FIXME: allow batches to be "pinned" to a particular assurance level or
      // prevented from going below that level -- and make it so that the
      // caller indicates whether the batch should be pinned or not, then
      // pinned batches need not be concerned with checking for changes -- only
      // unpinned batches need to be concerned ... *and* `entity` can store
      // batch IDs for the latest pinned and unpinned batches; pinned batches
      // will have a `minAssuranceForResolution` stored on them, and unpinned
      // batches will inherit the value from the entity; when creating an
      // unpinned batch, an extra check must be made to ensure that unpinned
      // batches were not concurrently invalidated

      // FIXME: change to `unpinnedBatchInvalidationCount`
      // ensure that `batchInvalidationCount` has not changed
      ...
      if(originalBatchInvalidationCount !== batchInvalidationCount) {
        // FIXME: are there use cases where we want to allow the token batch
        // to be created? do we want to create it but *only* with a higher
        // `minAssuranceForResolution`? ... maybe we want to invalidate token
        // batches when lowering `minAssuranceForResolution` so that there is
        // a refresh requirement ... then we don't have to worry about other
        // batches at that time ... maybe we want to create token batches
        // based on the `minAssuranceForResolution` given at creation time
        throw new BedrockError(
...
async function _getUnfilledBatch({internalId, batchVersion}) {
  ...
  while(true) {
    ...
    if(!entityRecord || !entityRecord.entity.batchId) {
      // entity does not exist, either it is being created concurrently,
      // or it has expired; if entity does exist but batch ID is `null` then
    ...
    const {entity: {batchId}} = entityRecord;
    ...
    if(!record ||
      ...
      // or no tokens left to issue in the batch... in all cases, there is
      // no usable open batch, so schedule an update to set the open token
      // batch to `null` (provided it has not changed concurrently) and return
      // that there is no usable open batch
      entities.setOpenTokenBatchId(
        {internalId, batchId: null, oldBatchId: batchId}).catch(() => {
        // ignore errors here, we do not require an update
      });
      return {unfilledRecord: null, entityRecord};
    }
...
async function _insertBatch({
  ...
  // FIXME: we MUST check to see if the registration associated with
  // `internalId` has a matching `minAssuranceForResolution`, otherwise
  // we must mark the token batch as invalid and try again... but this
  // requires looking up the registration based on `internalId` which
  // does scatter-gather and is slow! ugh!
  // FIXME: do we want another collection that shards based on `internalId`
  // and tracks `minAssuranceForResolution` separately? and then make that
  // the one location for it ... so it's not tracked in registrations nor
  // in token batches? we could fetch it at the same time we fetch a token
  // batch ... and would need to rethink how to handle invalidating token
  // batches ... it would probably still be the case that we'd need to look
  // up the value for changes after creating a new token batch

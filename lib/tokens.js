...
export async function _createTokens({
  internalId, attributes = new Uint8Array(), tokenCount,
  minAssuranceForResolution = 2,
  tokenizer, batchVersion,
  externalIdHash, registerPromise, newRegistration
...
// FIXME: move token parsing and payload creation to a separate file
...
async function _getOpenBatch({
  ...
  while(true) {
    ...
    if(!batchRecord) {
      ...
      */
      ...
      if(!entityRecord && !newRegistration) {
        ...
        // to loop and call again; there is no need to await any updates to
        // related documents since a new entity will be used and this code will
        // be called again
        return result;
      }
    ...
    // FIXME: move this to top of loop and switch conditional to improve
    // readability (reduce indentation on the larger code block above)
...
// FIXME: move pairwise token functions into their own file
...
async function _upsertPairwiseToken({internalId, requester}) {
  ...
  // then remove the `createPairwiseToken` thing ... ensure we do not
  // retrieve nor overwrite a pairwise token if `requester` does not match

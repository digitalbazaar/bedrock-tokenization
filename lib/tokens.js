...
/**
 * Invalidates the token batch by setting the expiration to now, but leaves
 *   the record intact so previously resolved tokens are trackable.
 *
 * @param {object} options - Options to use.
 ...
 * @returns {Promise<boolean>} Returns true if update occurred.
 */
export async function expireTokenBatch({internalId}) {
  const query = {
    'tokenBatch.internalId': internalId,
    'tokenBatch.remainingTokenCount': {$gt: 0}
  };
...
async function _getBatch({id}) {
  const query = {
    'tokenBatch.id': id,
    'tokenBatch.expires': {$gte: new Date}
  };
  const projection = {_id: 0};
  const collection = database.collections['tokenization-tokenBatch'];
  const record = await collection.findOne(query, {projection});

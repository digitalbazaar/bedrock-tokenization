...
export async function resolve({requester, token, levelOfAssurance}) {
  ...
  while(true) {
    ...
    // await any parallel potential entity record lookup
 ...
 * to expiration. The remaining token count is set to 0 so no new tokens can be
 * erroneously issued from any invalid unpinned batch.
 *
 * @param {object} options - Options to use.
 * @param {string} options.internalId - The internal ID of the entity.
 *
 * @returns {Promise<boolean>} Returns true if update occurred.
 */
export async function invalidateTokenBatches({internalId}) {
  // first indicate that token batches are about to be invalidated so that
  // concurrently created unpinned token batches can be invalidated by other
  // processes
  await entities.incrementBatchInvalidationCount({internalId});

  const query = {
    'tokenBatch.internalId': internalId,
    'tokenBatch.minAssuranceForResolution': -1
  };
  const $set = {
    'meta.updated': Date.now(),
...
async function _insertBatch({
  id, internalId, batchVersion, tokenCount, minAssuranceForResolution = -1
}) {
...
async function _claimTokens({tokenBatch, tokenCount}) {
  ...
  if(newRemainingTokenCount === 0) {
    ...
    await entities.setOpenTokenBatchId({

...
async function _getOpenBatch({
  ...
  // FIXME: do we need to return `minAssuranceForResolution` from here?
  while(true) {
    ...
    // FIXME: sequence number has been removed... figure out what applies
    // here if anything ... may not matter anymore

    // FIXME: *if* no tokens have been claimed in the batch, then we MUST
    // check the registration to ensure the sequence number hasn't changed
    // which should be done concurrently with claiming tokens ... HOWEVER,
    // doing a lookup on `internalId` does scatter-gather and is slow! ugh!
...
async function _getUnfilledBatch({internalId, batchVersion}) {
  ...
  while(true) {
    ...
    if(!record ||
      ...
      // FIXME: can we avoid removing it so it can be used to also store
      // `minAssuranceForResolution`? ... it would become a single record

...
import {computeIndexFields} from './util.js';
...
async function _createBatch({internalId, batchVersion, tokenCount = 0}) {
  ...
  const batchIdBytes = await _randomBytesAsync(batchIdSize);
  const batchId = base64url.encode(batchIdBytes);
  ...
  await _createOpenBatchReference({internalId, batchId, batchIdBytes});
...
async function _getUnfilledBatch({internalId, batchVersion}) {
  ...
  while(true) {
    ...
    const query = computeIndexFields({
      fieldName: 'tokenBatch.id',
      value: base64url.decode(batchId)
    });
...
async function _createOpenBatchReference({internalId, batchId}) {
  ...
  let record = {
    ...
    ...computeIndexFields({
      ...
      value: base58.decode(internalId.substr(1))
    }),
    internalId,
    batchId
  };

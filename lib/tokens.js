...
appContextMap.set(CIT_CONTEXT_URL, 0x8000);
...
export async function create(
  ...
  if(typeof internalId === 'string') {
    internalId = Buffer.from(base58.decode(internalId));
  }
...
export async function resolveToInternalId({token}) {
  ...
  const x = base58.encode(internalId.buffer);
  return {internalId: x};
}
...
async function _createToken({
  ...
  if(!(tokenBatch.id instanceof Buffer)) {
    tokenBatch.id = tokenBatch.id.buffer;
    tokenBatch.internalId = tokenBatch.internalId.buffer;
  }
  ...
  const token = await cborldEncode(
    {appContextMap, jsonldDocument, documentLoader});
  return token;
}
...
async function _parseToken({token}) {
  ...
  try {
    const {type, payload, meta} = await cborldDecode(
      {appContextMap, cborldBytes: token, documentLoader});
...
async function _createBatch({internalId, batchVersion, tokenCount = 0}) {
  ...
  const id = await _generateBatchId({batchIdSize});
...
async function _getOpenBatchReference({internalId}) {
  ...
  const projection = {_id: 0};
  const collection = database.collections['tokenization-openTokenBatch'];
  const y = await collection.findOne(query, {projection});
  return y;
}
...
async function _generateBatchId({batchIdSize}) {
  // _randomBytesAsync declared here to support stubbing in the test suite.
  const _randomBytesAsync = promisify(crypto.randomBytes);
  const randomBytes = await _randomBytesAsync(batchIdSize);
  randomBytes[0] %= 24;

...
async function _getOpenBatch({
  ...
  while(true) {
    ...
    if(!batchRecord) {
      ...
      If a token batch has its own `minAssuranceForResolution`, then it is
      a locked value that does not change. These types of token batches are
      referred to as pinned batches for this reason. Whenever we create a new
      pinned batch, we do not need to perform any additional checks before
      issuing tokens from it.

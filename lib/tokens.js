...
export async function create(
  ...
  if(attributes.length > 18) {
    throw new TypeError('"attributes" maximum size is 18 bytes.');
  }
...
async function _createToken({
  ...
  // attributes (up to 18 bytes max, but affects token size)
  ...
  // wrap `toWrap` as if it were a key, output is 40 bytes
  ...
  // 40 bytes: wrapped data
  // 18 bytes: attributes (max of 18, can be 0 to reduce token by 18 bytes)
  // = 78 byte token max, 60 bytes minimum
...
async function _parseToken({token}) {
  ...
  const minimumSize = 4 + batchSaltSize + 40;
  // 18 bytes is maximum size for attributes
  const maximumSize = minimumSize + 18;
  ...
  const wrapped = new Uint8Array(token.buffer, offset += batchSaltSize, 40);
  const attributes = new Uint8Array(
    token.buffer, offset += 40, token.length - minimumSize);

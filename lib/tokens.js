...
async function _getUnfilledBatch({
  ...
  while(true) {
    ...
    if(!record ||
      ...
      // no usable batch; schedule an update to set the open token batch to
      // `null` (provided it has not changed concurrently) and return that
      // there is no usable open batch
      entities._setOpenTokenBatchId({
...
async function _claimTokens({tokenBatch, tokenCount, batchInvalidationCount}) {

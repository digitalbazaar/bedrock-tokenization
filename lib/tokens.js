...
/**
 * Creates `tokenCount` many unique tokens using the given `tokenizer`, each
 * bound to the same given `internalId`, having the given authenticated but
...
export async function create({
  tokenizer, internalId, attributes = new Uint8Array(), tokenCount
} = {}) {
...
async function _parseToken({token}) {
  ...
  const minimumSize = 4 + batchSaltSize + 32;
  ...
  let offset = token.byteOffset + 4;
  const salt = new Uint8Array(token.buffer, offset, batchSaltSize);
  const wrapped = new Uint8Array(token.buffer, offset += batchSaltSize, 32);
  const attributes = new Uint8Array(
    token.buffer, offset += 32, token.length - minimumSize);
  ...
  const index = unwrapped[offset += batchIdSize];
  const aadSize = unwrapped[offset += 1];
  ...
  const toCompare = new Uint8Array(
    unwrapped.buffer, offset += 1, attributes.length);

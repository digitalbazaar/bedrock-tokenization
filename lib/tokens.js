...
const {
  encode: cborldEncode, decode: cborldDecode
} = require('@digitalbazaar/cborld');
const {constants: citConstants, documentLoader} = require('cit-context');
...
export async function resolve({
  ...
  while(true) {
    ...
    if(resolvedList.get(index)) {
      ...
      if(requesterList) {
        ...
        if(bs.get(index)) {
          ...
          // if permitted, return pairwise token w/o running any other checks
          // from below (e.g., invalidated batch, LOA)
    ...
    if(tokenRecord) {
      ...
      ({pairwiseToken: {value: pairwiseToken}} = await _upsertPairwiseToken(
    ...
    // finally, return pairwise token and internal ID
    return {pairwiseToken, internalId};
  }
}

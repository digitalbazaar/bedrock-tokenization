...
export async function createTokens({
  ...
  // FIXME: how to avoid erroneously setting `expires` on an existing
  // registration record or entity record -- without also triggering
  // deoptimizations in index use, etc.?
  // using an array for the `update` will use the aggregation pipeline which
  // could be used to do a conditional update, at some unknown perf cost:
  /*db.foo.update({id: 3}, [{
    $set: {
      'something.expires': {
        $cond: {
          if: {
            $eq: [{$type: '$something.expires'}, 'missing']},
            then: '$something.expires',
            else: {$max: new Date() / expiresValue}
          }
        }
      }
    }]);*/
  registrationsExpire = true
} = {}) {
  ...
  assert.bool(registrationsExpire, 'registrationsExpire');
  ...
  while(tokens.length < tokenCount) {
    ...
    const {tokenBatch, startIndex, claimedTokenCount} = await _getOpenBatch({
      ...
      registerPromise, newRegistration, registrationsExpire
    });
...
async function _createBatch({
  ...
  minAssuranceForResolution, externalIdHash, registrationsExpire
}) {
  ...
  // entity record will not be updated using the `expires` value -- similarly,
  // if `registrationsExpire` is set to false, the entity will not be updated)
  const expires = _getTokenBatchExpires({batchVersion});

  /* Note: We insert the token batch and also update the entity record's open
  token batch ID and `expires` value (if set) concurrently. We do the entity
  record update regardless of whether the token batch is entirely consumed in
  order to ensure that the entity record's `expires` value (if set) is always
  at least as long as the valid token batches associated with it. We also
  ensure that any associated document registration records are refreshed
  (`expires` extended, if set) prior to setting the open batch ID -- ensuring
  that no tokens will be issued from an open batch that could expire prior to
...
async function _getOpenBatch({
  ...
  registerPromise, newRegistration, registrationExpires
}) {
  ...
  value (if set) and, if an entity record already exists, any associated
  document registration records' `expires` values. This ensures all these
  records will persist at least as long as any new valid token batch. Note that
...
async function _refreshDocumentRegistrationsThenSetOpenBatchId({
  ...
  // note: any registration record w/o `expires` set will be unaffected
  if(externalIdHash) {
  ...
  match by not passing it here), ensuring its `expires` field (if set) is
  updated. */
  return entities._setOpenTokenBatchId({
    internalId, batchId, batchInvalidationCount,
    minAssuranceForResolution,
    expires: registrationsExpire === false ? undefined : expires
  });
}

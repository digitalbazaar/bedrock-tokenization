...
import {MAX_EXPIRATION_DATE} from '../helpers.js';
...
export async function createTokens({
  ...
  registerPromise, registerOptions, newRegistration
} = {}) {
  ...
  assert.optionalObject(registerOptions, 'registerOptions');
  assert.optionalBool(newRegistration, 'newRegistration');
...
async function _createBatch({
  ...
  minAssuranceForResolution, externalIdHash, pairwiseTokenExpires
}) {
  ...
  // entity record will not be updated using the `expires` value); the
  // expiration date for the batch will also be used for any pairwise tokens
  // generated from it, unless another pairwise expiration value is given
  const expires = _getTokenBatchExpires({batchVersion});
  if(pairwiseTokenExpires === undefined) {
    pairwiseTokenExpires = expires;
  } else if(pairwiseTokenExpires instanceof Date) {
    pairwiseTokenExpires = new Date(Math.max(pairwiseTokenExpires, expires));
  }

  /* Note: We insert the token batch and also update the entity record's open
  token batch ID and any `expires` value concurrently. We do the entity record
  update regardless of whether the token batch is entirely consumed in order to
  ensure that the entity record will not expire before the valid token batches
  associated with it. We also ensure that any associated document registration
  records are refreshed (`expires` extended as needed) prior to setting the
  open batch ID -- ensuring that no tokens will be issued from an open batch
  that could expire prior to document registration records.
  ...
  const [record] = await Promise.all([
    _insertBatch({
      ...
      expires, batchInvalidationCount, pairwiseTokenExpires
    }),
...
async function _getOpenBatch({
  ...
  /* Note: `_createBatch` always attempts to increment the entity record's
  `expires` value to ensure it persists at least as long as the created batch,
  and, if an entity record already exists, any associated document registration
  records' `expires` values are similarly updated for the same purpose. Also,
  the `expires` field from an existing entity record is stored along with the
  token batch to inform the initial `expires` field of any pairwise tokens
  later created during resolution.

  In the case that no entity record exists, registration must be occurring
  concurrently as checked above, which means at least one document registration
  record with a sufficiently long `expires` value is being inserted. The TTL
  for this registration is also passed along, allowing it to be used to compute
  the an `expires` field to be stored with the token batch, again, to be used
  with any new pairwise tokens. */

  // use `MAX_EXPIRATION_DATE` for pairwise tokens if the entity record or
  // registration also indicates it; otherwise use `undefined` to let
  // `_createBatch()` set the value according to the batch parameters
  const pairwiseTokenExpires = (
    entityRecord?.entity.expires.getTime() === MAX_EXPIRATION_DATE.getTime() ||
    registerOptions?.ttl === -1) ? MAX_EXPIRATION_DATE : undefined;
...
async function _getUnfilledBatch({
  ...
  if(!entityRecord || !entityRecord.entity.openBatch[key]) {
    // if entity does not exist, either it is being created concurrently, or it
  ...
  if(!record ||
    ...
    // scheduling an update to set open token batch to `null` is only an
...
async function _insertBatch({
  ...
  expires, minAssuranceForResolution = -1, pairwiseTokenExpires
}) {
...
async function _refreshDocumentRegistrationsThenSetOpenBatchId({
  ...
  // note: any registration record w/o `expires` set will be unaffected, but
  // this is presently no records
  if(externalIdHash) {

...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  /* Note: This is a backwards-compatible method to modify the existing TTL
  index option `expireAfterSeconds`. It adds a grace period prior to the
  removal of records. This provides better resiliency for decision making
  processes that must retrieve records, make decisions about them (based on
  their existence) and then update them to extend their expiration period.
  ...
  await database.db.command({
    collMod: 'tokenization-tokenBatch',
    index: {
      name: 'tokenBatch.expires_1',
...
export async function getBatch({id, explain = false} = {}) {
  ...
  if(!record) {
    ...
    // path and explain that in the notes here when finished... note that
    // unpinned token batches are never set to `null` to ensure that the latest
    // one that was created (with the latest expiration date) is always
    // expressed in the entity record -- which is different from any open
    // pinned token batch which gets set to null when used up
    // FIXME: also, should this check be done here or at the application layer?
    // ... if here, it should be documented that it is the default behavior and
    // there is no feature to change it yet due to lack of use cases
...
async function _getOpenBatch({
  ...
  const {tokenBatch} = await _createBatch({
    ...
    externalIdHash: entityRecord?.externalIdHash
  });
...
async function _refreshDocumentRegistrationsThenSetOpenBatchId({
  ...
  if(externalIdHash) {
    await documents.refreshAll({externalIdHash, expires});
  }

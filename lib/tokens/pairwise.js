...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  await database.createIndexes([{
    ...
    options: {
      ...
      unique: false,
      background: false,
      // grace period of 24 hours; see other TTL indexes in this module for
      // comments on the rationale for this value
...
async function _update({internalId, requester, expires}) {
  const query = {
    'pairwiseToken.internalId': internalId,
    'pairwiseToken.requesterId': requester,
    // only find and update pairwise token if it expires before `expires`
    'pairwiseToken.expires': {$lt: expires}
  };
  const update = {
    $set: {

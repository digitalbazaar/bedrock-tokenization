...
export async function upsert({
  ...
  if(optimizeForExisting) {
    ...
    record's expiration, if set, is updated. If the first `get` operation fails
    to find a result, we create one, looping if another process is running that
    creates one first. */
...
async function _create({internalId, requester, expires}) {
  ...
  if(expires !== undefined) {
    record.pairwiseToken.expires = expires;
  }
  try {
...
async function _update({internalId, requester, expires}) {
  ...
  if(expires !== undefined) {
    update.$max = {'pairwiseToken.expires': expires};
  }

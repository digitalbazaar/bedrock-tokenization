...
import {assertTtl} from '../helpers.js';
import {tokenizers} from '@bedrock/tokenizer';

// expose public functions
export {
  resolve, resolveToEntity, resolveToInternalId, resolvePairwiseToken
} from './resolve.js';
export {
...
export {
  get as getPairwiseToken,
  upsert as upsertPairwiseToken
} from './pairwise.js';
...
export async function registerDocumentAndCreate({
  ...
  assertTtl({ttl: registerOptions.ttl});
  ...
  before the writes are considered safe (which is the recommended behavior).
  ...
  if(registerOptions.ttl !== -1) {
    registerOptions.ttl = Math.max(batchVersionTtl, registerOptions.ttl);
  };
  ...
  while(true) {
    ...
    const [registrationResult, createResult] = await Promise.all([
      ...
      _createTokens({
        internalId, attributes, tokenCount, minAssuranceForResolution,
        tokenizer, batchVersion, registerPromise, registerOptions,
        newRegistration
      })
    ]);

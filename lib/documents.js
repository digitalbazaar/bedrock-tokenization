 ...
 * record without `expires` set will be unaffected.
 *
...
export async function refreshAll({
  ...
  const query = {
    ...
    // if `registration.expires` does not exist, the query will not match
...
/**
 * Retrieves the registration record for an internalId.
 ...
 * system before it is stored. This approach ensures that a stolen database on
 * its own will not reveal the correlation between a particular `externalId`
 * and an `internalId`. This `internalId` will persist and be reused across all
 * documents registered using the same `externalId`.
 *
 * The `document` can be optionally stored (but only once it has been encrypted
 * first). To encrypt and store the document, specify either the `recipients`
 * or the `recipientChain` options and do not set `store: false`. To avoid
 * storing the encrypted document, set `store: true` and do not set any
 * recipient options; any other combinations will result in an error.
 *
 * The document registration record will also be marked for expiration using
 * the given `ttl` parameter. Once all of the registered document records for a
 * given `externalId` have expired, the `externalId` and its associated
 * `internalId` will be removed such that if the same `externalId` is given in
 * the future, a new `internalId` will be generated. That a new `internalId`
 * will be used for future interactions with the same `externalId` is another
 ...
 * @param {boolean} [options.store] - Set to `false` and provide no recipient
 *   options to prevent encryption and storage of the full document, i.e., the
 *   full document will be discarded and only its hash and other metadata will
 *   be stored.
 *
...
export async function register({
  ...
  // FIXME: allow `ttl` to be undefined -- or require it to be set to
  // `Infinity`? or require it to be set to `false`?
  ...
  if(store === false) {
    if(recipientChain || recipients) {
      throw new TypeError(
        '"store" must not be set to "false" when also providing "recipient" ' +
        'or "recipientChain".');
    }
  } else if(recipientChain) {
    if(recipients) {
      throw new TypeError(
  ...
  while(true) {
    ...
    const jwe = store === false ?
      undefined :
      await _encrypt({document, recipients, recipientChain});
...
async function _insertRegistration({
  ...
  if(ttl !== undefined) {
    record.registration.expires = new Date(now + ttl);
  }
  if(creatorHash) {
...
export async function _refresh({
  ...
  if(ttl !== undefined) {
    ...
    // to accomplish this and we cannot include `expires` in the query because
    // we want to find the document even if we don't update `expires`
    update.$max = {'registration.expires': new Date(now + ttl)};
  }
  if(creatorHash) {

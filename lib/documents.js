 ...
 * the `internalId` is generated randomly vs. computed as an HMAC value from
 * the `externalId`.
 *
...
export async function getRegistration({internalId, explain = false} = {}) {
  ...
  if(record) {
    // explicitly check `record.expires` against current time to handle
    // cases where the database record just hasn't been expunged yet
    const now = new Date();

...
export async function register({
  ...
  const [externalIdHash, hash, creatorHash] = await Promise.all(promises);

  while(true) {
    // 3. Attempt to refresh the expiration for the document.
    if(await _refresh({externalIdHash, hash, ttl, creatorHash})) {
...
async function _insertDocument({
  ...
  if(creatorHash) {
    record.document.creatorHash = creatorHash;
  }
  try {
...
async function _refresh({externalIdHash, hash, ttl, creatorHash}) {
  const query = {
    'document.externalIdHash': externalIdHash,
    'document.hash': hash
  };
  ...
  if(creatorHash) {
    update.$addToSet = {
      'document.creatorHash': creatorHash
    };
  }
  const collection = database.collections['tokenization-document'];

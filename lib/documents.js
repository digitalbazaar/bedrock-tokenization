...
export async function register({
  ...
  while(true) {
    ...
    if(newRegistration !== true) {
      ...
      if(internalId) {
        upsertEntityPromise = entities.upsert(
    ...
    //    completes.
    // FIXME: determine if we want the `ttl` to be longer for this
    const upsertEntityPromise = entities.upsert(
      {internalId, ttl, minAssuranceForResolution});

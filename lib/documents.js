...
import {setMinAssuranceForResolution} from './tokens.js';
...
export async function register({
  ...
  while(true) {
    ...
    if(newRegistration !== true &&
      ...
      // this will only lower the minAssuranceForResolution if there is a valid
      // registration. It will not be expired since it was just refreshed.
      await setMinAssuranceForResolution({
...
/**
 * Retrieves a registration record by `externalId`.
 *
 * @param {object} options - Options to use.
 ...
 */
export async function getRegistrationbyExternalId({externalId}) {
  const tokenizer = await tokenizers.getCurrent();
  const {hmac} = tokenizer;
  const promises = [_hmacString({hmac, value: externalId})];
  const [externalIdHash] = await Promise.all(promises);
  const query = {

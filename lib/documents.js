...
import {updateBatches} from './tokens.js';
 ...
 * @param {boolean} [options.updateTokenBatches] - Specify whether or
 *   not to update token batches.
 *
...
export async function register({
  ...
  documentHash, creatorHash, newRegistration, updateTokenBatches
} = {}) {
  ...
  while(true) {
    ...
    if(!newRegistration &&
      ...
      const {registration} = await _getRegistrationRecord({
        externalIdHash, documentHash
      });
      // this will only update the tokenBatches if there is a valid
      // registration. It will not be expired since it was just refreshed.
      if(updateTokenBatches) {
        await updateBatches({
          internalId: registration.internalId, minAssuranceForResolution
        });
      }
    ...
    // throw an error if there is a flag to update token batches to lower the
    // minAssuranceForResolution but there is no valid registration
    if(updateTokenBatches && minAssuranceForResolution === 1) {
      const details = {
        httpStatusCode: 404,
        public: true
      };
...
async function _refresh({
  externalIdHash, documentHash, ttl, creatorHash, minAssuranceForResolution
}) {
  const query = {
    'registration.externalIdHash': externalIdHash,
    'registration.documentHash': documentHash

...
import {updateBatch} from './tokens.js';
...
export async function register({
  ...
  minAssuranceForResolution = 2, ttl, creator, tokenizer, externalIdHash,
  ...
  while(true) {
    ...
    if(!newRegistration &&
      ...
      const {registration} =
        await _getRegistrationRecord({externalIdHash, documentHash});
      await updateBatch({
...
/**
* Raises the minimum assurance for resolution on both the registration and
* token batches.
*
* @param {object} options - Options to use.
* @param {string} options.externalId - An application-specific string that
...
*   assurance required for token resolution.
*
* @returns {Promise<Buffer>} The internalId of the registration record.
*/
export async function raiseAssurance({
  externalId, document, creator, ttl, minAssuranceForResolution
} = {}) {
  ...
  return registration.internalId;
}
...
async function _refresh({
  externalIdHash, documentHash, ttl, creatorHash, minAssuranceForResolution = 2
}) {
  const query = {
    'registration.externalIdHash': externalIdHash,
    'registration.documentHash': documentHash

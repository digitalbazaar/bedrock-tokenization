...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  await promisify(database.createIndexes)([{
    ...
    // fields: {'registration.externalIdHash': 1,
    //'registration.documentHash': 1},
    fields: {
      'registration.externalIdHash_0': 1,
      'registration.externalIdHash_1': 1,
      'registration.externalIdHash_2': 1,
      'registration.documentHash': 1
    },
...
async function _getRegistrationRecord({externalIdHash, documentHash}) {
  const query = {
    ...
    ...computeIndexFields({
      fieldName: 'registration.externalIdHash',
      value: externalIdHash,
    }),
...
async function _insertRegistration({
  ...
  let record = {
    ...
    registration: {
      internalId,
      // full externalIdHash is still being recorded, but will not be used
      // for queries
      externalIdHash,
      ...computeIndexFields({
        fieldName: 'externalIdHash',
        value: TEXT_ENCODER.encode(externalIdHash),
      }),
      documentHash,
      tokenizerId,
      jwe,

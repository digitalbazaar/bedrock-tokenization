...
export async function register({
  ...
  minAssuranceForResolution = 1, ttl, creator, tokenizer, externalIdHash,
  ...
  while(true) {
    ...
    if(newRegistration !== true) {
      // FIXME: optimistically upsert the entity in parallel here... but
      // we need to check afterwards to ensure that the internal ID matches
      // what is returned in the record from `_refresh` -- because, if not,
      // we must upsert again ... or just say this in the documentation
      // and push it outside of this function since we already do that
      // in `registerDocumentAndCreate`
      if(internalId) {
        // FIXME:
      }
    ...
    // FIXME: need to call upsert `entity` in parallel and await it after
    ...
    try {
      const record = await _insertRegistration({
        ...
        jwe,
        // FIXME: remove this, it goes in entity record only
 ...
 * registration associated `internalId`. Exported for internal use and testing
 * purposes only.
 *
 * @param {object} options - Options to use.
 * @param {string} options.externalIdHash - The external ID hash.
...
export async function _setMinAssuranceForResolution({
  externalIdHash, documentHash, minAssuranceForResolution
}) {
  const query = {
    'registration.externalIdHash': externalIdHash,
    'registration.documentHash': documentHash,
  ...
  const $set = {
    ...
    'registration.minAssuranceForResolution': minAssuranceForResolution
  };
...
async function _refresh({
  ...
  // FIXME: remove `minAssuranceForResolution` here?
  minAssuranceForResolution
}) {
  const query = {
    'registration.externalIdHash': externalIdHash,
  ...
  // FIXME: remove this, only set in entity record
  if(minAssuranceForResolution !== undefined) {
    update.$set['registration.minAssuranceForResolution'] =
      minAssuranceForResolution;
  }
  const collection = database.collections['tokenization-registration'];
  const result = await collection.updateOne(

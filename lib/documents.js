...
// FIXME: is this export available from bedrock-mongodb, if not, make it so
...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  await promisify(database.createIndexes)([{
    ...
    fields: {
      'registration.longHash': 1,
    },
    // FIXME: is unique OK?
    // In this case we have reduced collision resistance so I would not
    // make it unique. If it's a real concern I'd consider a quick check for
    // the existence of the computed value before inserting and if it exists
    // checking for a duplicate then
...
export async function register({
  ...
  while(true) {
    ...
    const randomBytes = Buffer.from(await idGenerator.generate());
    randomBytes[0] %= 24;
    const n = Math.floor(Date.now() / (60000));
    randomBytes[1] = (n >>> 24) & 0xFF;
    randomBytes[2] = (n >>> 16) & 0xFF;
    randomBytes[3] = (n >>> 8) & 0xFF;
    randomBytes[4] = n & 0xFF;
    const internalId = randomBytes;
...
async function _getRegistrationRecord({externalIdHash, documentHash}) {
  const query = {
    'registration.longHash': _computeLongHash({externalIdHash, documentHash}),
...
async function _insertRegistration({
  ...
  let record = {
    meta,
    registration: {
      longHash: _computeLongHash({externalIdHash, documentHash}),
      internalId,
      externalIdHash,
      documentHash,
      tokenizerId,
...
function _computeLongHash({externalIdHash, documentHash}) {
  const skHash = crypto.createHash('md5')
    .update(externalIdHash + documentHash).digest('hex');
  return Long.fromString(skHash.substr(0, 16), 16);
}

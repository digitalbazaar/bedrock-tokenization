...
config.tokenization.defaultVersionOptions = {
  ...
  batches -- and those existing token batches would become invalidated if
  an application makes a call to check for invalid token batches when trying
  to reset an entity's `minAssuranceForResolution` to some value. While this
  is not a security problem, it could have a detrimental affect on UX. */
